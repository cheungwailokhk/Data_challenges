...
class Cralwer:
    ...
    def removePunctuation(self, text):
        ...
        return translator.sub(' ', text.replace("“", '').replace("„", "").replace("‘", "").replace("‚", ""))
    ...
    def plot_bar_x(self, labels, freq, xlabel, ylabel, title):
        ...
        plt.xticks(index, labels, fontsize=6, rotation=60)
        plt.title(title)
        plt.show()
        return
    ...
    def visualizeBigramsNetworks(self):
        ...
        for bigram, counts in bigram_dict.items():
            words = bigram.split(" ", 2)
            graph.add_edge(words[0], words[1], weight=(counts))
